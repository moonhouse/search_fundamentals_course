...
def autocomplete():
    ...
    if request.method == 'GET':  # a query has been submitted
        ...
        if prefix is not None:
            # This next line looked a bit too susceptible to injection to leave alone.
            index = "bbuy_products" if request.args.get("type", "queries") == "products" else "bbuy_queries" # If type == queries, this is an autocomplete request, else if products, it's an instant search request.
            ##### W2, L3, S1
            query_obj = { "suggest": { "autocomplete": { "prefix": prefix, "completion": { "field": "suggest", "skip_duplicates": True } } } }
